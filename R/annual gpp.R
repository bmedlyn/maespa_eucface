...
see.flx <- summaryBy(GPP + Ra + Trans + soil.e + volRing~ year + Ring,data = maespa.df,FUN=sum)
...
see.ring.sum <- summaryBy(GPP.sum + Ra.sum + Trans.sum + soil.e.sum + volRing.sum~ year+Ring,data = see,FUN=mean)
see.amb.sum <- summaryBy(GPP.sum + Ra.sum + Trans.sum + soil.e.sum + volRing.sum ~ year+c.treat,
                         data = see,FUN=mean,na.rm=TRUE)
