...
source("R/load.R")
source("R/warpar.R")
v.reduction <- 0.074 # -15% based on Anisworth and long 2005
for(i in c(1/1.6*(1-v.reduction),1/1.6*(1+v.reduction))){
  ...
  dir.create(file.path("output",paste0("vc",vj.ratio)))
  file.rename(from=fn.vec,
              to=file.path("output",paste0("vc",vj.ratio),fn.vec))
}