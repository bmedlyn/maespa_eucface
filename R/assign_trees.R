update.tree.f <- function(lai.test,lai.base){
  radius <- 12.5 #ring radius; m
  zero.position <- radius 
  srrounding.tree <- 8 * 10
  ...
  for (i in 1:6){
    ...
    maxnotrees <- 200
    replacePAR(cf, "notrees", "treescon", min(length(coord_H_D_byRing[[i]]$ratio)+srrounding.tree, maxnotrees))
    ...
    replaceNameList("plot", tf, 
                    vals=list(x0  =	0,
                              ...
                              yslope	=	0,
                              bearing	=	-90,
                              notrees	=	length(coord_H_D_byRing[[i]]$ratio)+srrounding.tree))
  }
...
#   add <- matrix(c(rep(-10,10),rep(-5,8),rep(30,8),rep(35,10),
#                   seq(-5,30,5),seq(0,25,5),seq(0,25,5),seq(-5,30,5),
#                   seq(-10,35,5),seq(-5,30,5),seq(-5,30,5),seq(-10,35,5),
#                   rep(35,8),rep(30,6),rep(-5,6),rep(-10,8)
#                   ),ncol = 2)
  # new method for a ring of trees
...
# #   coor.E <- cr.e.vec + zero.position  
    ...
    coor.N <- c(ls.15[[2]],ls.17[[2]]) + zero.position
    coor.E <- c(ls.15[[1]],ls.17[[1]]) + zero.position  
