...
library(zoo)
library(Maeswrap)
maespa.df.hr <- readRDS('cache/ca.df.rds')
maespa.df.hr$VPD <- getVPD(maespa.df.hr$RH,maespa.df.hr$TAIR)
mean(maespa.df.hr$TAIR[maespa.df.hr$PAR>1])
mean(maespa.df.hr$TAIR)
# maespa.df.hr <- readRDS("output/ambient/mastra and sap hr.rds")
# maespa.df.hr$ppfd <- maespa.df.hr$PAR
# maespa.df.hr$Date <- as.Date(maespa.df.hr$DateTime)C
...
# temp.df <- merge(maespa.df.hr,swc.50.df,by=c('Date'),all=TRUE)
# temp.1.df <- merge(temp.df,euc.sum.df,by=c('Date',"Ring"),all=TRUE)
temp.1.df <- maespa.df.hr
...
maespa.df.hr.r1$Vcmax <- 90 * (1-0.14 * maespa.df.hr.r1$VPD)
maespa.df.hr.r1$Jmax <- 1.7 * 90 * (1-0.14 * maespa.df.hr.r1$VPD)
maespa.df.hr.r1$Jmax[maespa.df.hr.r1$Jmax<10] <- 10
maespa.df.hr.r1$Vcmax[maespa.df.hr.r1$Vcmax<10] <- 10

maespa.df.hr.r1 <- maespa.df.hr.r1[maespa.df.hr.r1$PAR > 1,]
library(plantecophys)
photo.df <- Photosyn(g1 = 4.3,#maespa.df.hr.r1$g1,#Vcmax=maespa.df.hr.r1$Vcmax,Jmax = maespa.df.hr.r1$Jmax,
                     Vcmax = maespa.df.hr.r1$Vcmax,
                     Jmax = maespa.df.hr.r1$Jmax,
                     VPD = maespa.df.hr.r1$VPD, 
                     Ca = 550,#maespa.df.hr.r1$CA, 
                     # PPFD = maespa.df.hr.r1$PAR, 
                     PPFD = 1800,
                     Tleaf = maespa.df.hr.r1$TAIR,
                     gsmodel = c("BBOpti"),
...
photo.df$Ac.inc <- (photo.df$Ac / photo.df.a$Ac )
photo.df$Aj.inc <- (photo.df$Aj / photo.df$Aj.amb )
photo.df <- photo.df[photo.df$GS>0,]
par(mfrow=c(2,1))
par(mar=c(1,5,4,1))
...
mean(photo.df$Aj.inc,na.rm=T) * 0.8 + 
mean(photo.df$Ac.inc,na.rm=T) * 0.2
